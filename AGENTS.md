# Lalafo Printed CLI Agent Guide

This directory is a generated `lalafo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
lalafo-pp-cli doctor --json
lalafo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
lalafo-pp-cli which "<capability>" --json
lalafo-pp-cli <command> --help
```
...
```bash
lalafo-pp-cli <command> --help
lalafo-pp-cli <command> --dry-run --agent
```

...
func TestSearchAdsQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("ads", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchAds(query, 10)
		if err != nil {
			t.Fatalf("SearchAds(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchAds(%q) returned no results", query)
		}
	}
}
...
// TestUpsertBatch_CategoriesSearchableViaGenericFTS verifies that a
...
// non-FTS5 `--type categories` fallback), so populating it is the
...
func TestUpsertBatch_CategoriesSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// field is matchable regardless of which columns categories declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("categories", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "categories")
	}
}
...
// TestUpsertBatch_PopulatesParamsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed params table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesParamsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("params", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "params").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "params")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count params: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("params count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

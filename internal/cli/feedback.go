...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LALAFO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LALAFO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "lalafo-pp-cli/feedback")
...
When ` + "`LALAFO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LALAFO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lalafo-pp-cli feedback list
  lalafo-pp-cli feedback list --limit 5
  lalafo-pp-cli feedback list --json`,

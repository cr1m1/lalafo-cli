...
In local mode: searches locally synced data only.`,
  ...
  lalafo-pp-cli search "error timeout"

  # Force local search only
  lalafo-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  lalafo-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  lalafo-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v3/ads/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/v3/ads/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'lalafo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "ads":
				results, err = db.SearchAds(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAds(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search ads failed: %w", searchErr)
					}
